...
* We then obtain a minimum spanning tree from this graph, for convenience of visualization as well as for later comparison with a few other methods that purport to provide estimates of cell lineages.
...
It is generally recommended to leave the ```options``` field empty, which will trigger an interface guiding the user through the choice of parameters for the ECLAIR analysis at hand.
